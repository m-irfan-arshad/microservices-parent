...
if [ ! -f "docker-compose_dev.yml" ] && [ ! -f "docker-compose_dev.yaml" ] && [ ! -f "compose_dev.yml" ] && [ ! -f "compose_dev.yaml" ]; then
    ...
    echo "Expected one of: docker-compose_dev.yml, docker-compose_dev.yaml, compose_dev.yml, compose_dev.yaml"
    exit 1
fi

echo "✓ Docker Compose Dev file found"
echo ""

# Stop and remove containers
docker compose -f docker-compose_dev down -v
